...
def quantize_tensor(x, num_bits=8, min_val=None, max_val=None):
    ...
    cfg.writer.add_scalars("scale", {"scale": scale}, n_count)
    cfg.writer.add_scalars("zero_point", {"zero_point": zero_point}, n_count)
    n_count += 1
...
class Qconv2d_INT(torch.nn.Conv2d):
    ...
    def conv2d_forward(self, input, num_bits=8, min_val=None, max_val=None):
        ...
        q_input = quantize_tensor(input, num_bits=num_bits,
                                  min_val=torch.min(input), max_val=torch.max(input))

        q_weight = quantize_tensor(self.weight, num_bits=num_bits,
