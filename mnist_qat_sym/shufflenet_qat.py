...
from quantizer import Qconv2d, QLinear, Qconv2d_INT
...
def create_model():
    ...
    print(summary(model=Net.to("cuda"), input_size=(3, 224, 224), batch_size=cfg.batch_size,
                        ...
                        if isinstance(layers, nn.Sequential):
                            for k, layer in layers.named_children():
                                ...
                                if isinstance(layer, nn.Conv2d):
                                    ...
                                    if (False if layer.bias is None else True):
                                        ...
                                        #                 bias=False if layer.bias is None else True
                                        #                 )
                                        layer = Qconv2d_INT(in_channels=layer.in_channels,
...
def get_activation(name):
    def hook(model, input, output):
        ...
        # print(name, len(input), input[0].size())

        cfg.writer.add_histogram(name, input[0], cnt)
        cnt += 1
    return hook


def hook_forward_for_inpit(net):
    layer_Sequential = ['conv1', 'maxpool', 'stage2', 'stage3', 'stage4', 'conv5', 'fc']
    i = 0
    for outname, modules in net.named_children():
        if isinstance(modules, nn.Sequential):
            for name, module in modules.named_children():
                ...
                if isinstance(module, shufflenet.InvertedResidual):#stage2 + stage3 + stage4
                    for key, layers in module.named_children():
                        ...
                        if isinstance(layers, nn.Sequential):
                            for k, layer in layers.named_children():
                                # print("k:",k, type(k))
                                if isinstance(layer, nn.Conv2d):
                                    getattr((getattr(net, layer_Sequential[i])[int(name)]), str(key))[
                                        int(k)].register_forward_hook(
                                        get_activation(outname + name + key + k))
                                    print(getattr((getattr(net, layer_Sequential[i])[int(name)]), str(key))[int(k)])
                                    pass
                                    ...
                                    # getattr((getattr(net, layer_Sequential[i])[int(name)]), str(key))[int(k)].register_forward_hook(
                                    #     get_activation(outname + name + key + k))
                                    # print(getattr((getattr(net, layer_Sequential[i])[int(name)]), str(key))[int(k)])
                                    pass
                                else:
                    ...
                    if isinstance(module, nn.Conv2d):
                        # getattr(net, layer_Sequential[i])[int(name)] = module
                        getattr(net, layer_Sequential[i])[int(name)].register_forward_hook(
                            get_activation(outname + name))
                        print(getattr(net, layer_Sequential[i])[int(name)])
                        pass
                    elif isinstance(module, nn.BatchNorm2d):
                        pass
            ...
            if isinstance(modules, nn.Linear):
                ...
                getattr(net, layer_Sequential[i]).register_forward_hook(
                    get_activation(outname))

            elif isinstance(modules, nn.ReLU):
                # getattr(net, layer_Sequential[i]).register_forward_hook(
                #     get_activation(outname))
                # print(getattr(net, layer_Sequential[i]))
                pass
...
class Trainer:
    ...
    def run_INT(self):
        ...
        for epoch in range(1,  self.epochs + 1):
            ...
            print(epoch, cfg.start_QAT_epoch, self.QAT_flag, self.device, (epoch >= cfg.start_QAT_epoch and self.QAT_flag))
...
# hook_forward_for_inpit(Trainer.Net)
freeze(Trainer.Net)
print(Trainer.Net)
